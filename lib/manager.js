...
// 依赖注入 runSh/profile 工具（来自 shell.js），保持纯逻辑可测。
...
export function createManager(ctx, profile, shellTools) {
  const { runSh, profilePaths, readText, writeText, readManifest, updateManifest } = shellTools
  ...
  async function list() {
    const { home, dir } = await profilePaths(profile)
  ...
  async function install(spec) {
    ...
    const res = await runSh('dsh plugin --profile ' + profile + ' add ' + shellQuote(clean), 300000)
  ...
  async function uninstall(name) {
    ...
    const { dir } = await profilePaths(profile)
    const res = await runSh('dsh plugin --profile ' + profile + ' remove ' + shellQuote(name), 300000)
    const out = ((res.stdout || '') + '\n' + (res.stderr || '')).trim()
    if (res.exitCode !== 0) {
