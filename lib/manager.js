...
export function createManager(ctx, profile, fsTools) {
  ...
  async function search(query) {
    ...
    try {
      ...
      if (!res.ok) return { ok: false, message: 'GitHub 搜索失败 HTTP ' + res.status }
      ...
      return { ok: true, query: raw, items }
    } catch (e) {
      return { ok: false, message: 'GitHub 搜索失败: ' + String((e && e.message) || e) }
    }
  }
  ...
  return { list, install, uninstall, stop, start, setSettings, restart, search }
}
