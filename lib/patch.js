...
export function patchAppend(patchText, addition) {
...
export function patchHasLoad(patchText, name) {
  return patchText.indexOf('# ' + MARKER + ': load ' + name) !== -1
}

export function patchWithoutLoad(patchText, name) {
...
export function loadAddition(name, entryId) {
  const q = (s) => "'" + String(s).replace(/'/g, "'\\''") + "'"
