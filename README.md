...
- **GitHub 直跳**：卡片来源行末尾的链接，取自依赖声明或已安装包 `repository` 字段
- **自动重启**：安装 / 卸载成功后可选自动重启 dsh web（默认关闭，见下方「配置」）
- **分组与搜索**：内置 / 三方分组（可折叠），组内按状态分「运行中 / 已停用 / 失效」子组，支持模糊搜索
